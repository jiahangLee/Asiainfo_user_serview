...
public class LoginController {
    ...
    public String login(HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            ...
            //List<GrantedAuthority> authorities = AuthorityUtils.createAuthorityList("ROLE_USER", "ROLE_ADMIN","ROLE_TRUSTED_CLIENT");
            ...
            if(!authentication.isAuthenticated()){
                throw new CredentialException("AuthUser could not be authenticated");
            }else{
       ...
       // Map<String, String> authParams = getAuthParameters(defaultSavedRequest);
        ...
        if(defaultSavedRequest.getParameterMap().get(OAuth2Utils.REDIRECT_URI)!=null&&defaultSavedRequest.getParameterMap().get(OAuth2Utils.REDIRECT_URI).length>0){
            redirectUrl = defaultSavedRequest.getParameterMap().get(OAuth2Utils.REDIRECT_URI)[0];
        }
