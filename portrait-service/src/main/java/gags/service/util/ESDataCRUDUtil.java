...
//          createEducationIndexTypes("201803");
//        createIndexTypesApp("201711");
//        createIndexTypes("201712");
//        createIndexTypesFlow("201712");
//        createIndexTypesDVB("201712");
//        createIndexChannelTypes("201712");
//        createIndexTypeRetention("201712");
//        createIndexTypeSave("201711");
//        createIndexTypeAdvert("201711");
        // 删除types数据
        //deleteData();
...
//        String value = "20171115";
        String areaName = "1010";
        String l_date = "20180209";
...
//        client.admin().indices().create(request);
        ...
        for (String business : businessGroup1) {
            for (String period : periods) {
                for (String rank : regionRanks) {
                    String esBusinessUserTableName = "t_"+ rank +"_" + business + "_channel_" + period+"_three";
                    PutMappingRequest mapping = Requests.putMappingRequest(index).type(esBusinessUserTableName).source(getMapping());
                    client.admin().indices().putMapping(mapping).actionGet();
                }
            }
        ...
        for (String period : periods2) {
            for (String rank : regionRanks) {
                String esBusinessUserTableName = "t_" + rank + "_vod_program_" + period + "three";
...
//        client.admin().indices().create(request);
        ...
        for (String business : businessGroup1) {
            for (String period : periods) {
                for (String rank : regionRanks) {
                    String esBusinessUserTableName = "t_"+ rank +"_" + business + "_program_" + period+"_three";
                    PutMappingRequest mapping = Requests.putMappingRequest(index).type(esBusinessUserTableName).source(getDVBMapping());
                    client.admin().indices().putMapping(mapping).actionGet();
                }
            }
