...
public class ESDataUtil {
    ...
    public static void makeESDataMap(String[] indices, List<Map<String, Object>> data, String tableName, List<String> newKpis, QueryBuilder qb, @RequestParam String period) {
        ...
        if (newIndices.length > 0) {
            ...
            makeKpiDataMap(data, response, period);
        }
     ...
     * @param period
     */
    public static void makeKpiDataMap(List<Map<String, Object>> data, SearchResponse response, String period) {
        int rowNum = 1;
        out:
        for (SearchHit hit : response.getHits().getHits()) {
            ...
            for (Map.Entry<String, SearchHitField> entry : fieldEntry) {
                ...
                String kpiValue = entry.getValue().getValue();
                if(BaseDataUtil.getFields().contains(kpiName) && StringUtil.isEmpty(kpiValue)){
                    continue out;
                }
            }
            for (Map.Entry<String, SearchHitField> entry : fieldEntry) {
                ...
                if (entry.getValue().getName().equals("statistic_time")) {
                    try {
                        ...
                        kpiMapData.put(kpiName, getDateStr(period,statisticTime));
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                } else {
     ...
     * @return
     */
    public static String getDateStr(String period, Long value) {
        ...
        switch (period) {
            ...
            case "60":
                ...
                return kpiDateFormat.format(value);
            case "day":
                kpiDateFormat = new SimpleDateFormat("yyyy-MM-dd");
                return kpiDateFormat.format(value);
            case "week":
               return DateUtil.getWeekCnvDate(value);
            case "halfmonth":
                return DateUtil.getHalfMonthCnvDate(value);
            case "month":
                return DateUtil.getMonthCnvDate(value);
            case "quarter":
                return DateUtil.getQuarterCnvDate(value);
            case "year":
                return DateUtil.getYearCnvDate(value);
            default:
                kpiDateFormat = new SimpleDateFormat("yyyy-MM-dd");
                return kpiDateFormat.format(value);
        }
